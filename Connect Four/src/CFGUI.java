...
/**
 * A CFGUI is a graphical window designed for the game of connect four. It knows
 * how to display player names, and the board. MADE IT SO TOP ROW IS board[6]
 * and last row is board[0]
 */
public class CFGUI extends JFrame
{
	...
	/**
	 * Re-draw the window - this method should be called whenever there is a
	 * change to the board.
	 */
	public void redraw()
	{
		this.repaint();
		/*
		 * try { Thread.sleep(500); } catch (InterruptedException e) { // should
		 * never happen }
		 */
	}

	/**
	 * Prints which player one or tie if tie id = 1 if player 1 won, 2 if player
	 * 2 won, -1 if its a tie.
	 */
	...
	/**
	 * Close the window (remove it from the screen) - this method should be
	...
	public void reset(Board b, Player p1, Player p2)
	{
		b.reset();
		p1.reset();
		p2.reset();
